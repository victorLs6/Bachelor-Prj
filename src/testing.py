...
def bounded_mutation_with_depth(individual, mu=0, sigma=0.15, indpb=0.15):
    ...
    if random.random() < indpb:
        ...
        individual[num_layers + 1] = max(
            0.7, min(0.95, individual[num_layers + 1])
        )
    ...
    if random.random() < indpb:
        ...
        individual[num_layers + 2] = max(
            0.001, min(0.005, individual[num_layers + 2])
        )
    ...
    if random.random() < indpb:
        ...
        individual[num_layers + 3] = max(
            0.0, min(0.2, individual[num_layers + 3])
        )
...
def evaluate_model_with_depth(individual):
    ...
    try:
        ...
        num_layers = int(individual[0])
        layer_sizes = [
            max(32, min(256, int(round(individual[i]))))
            for i in range(1, num_layers + 1)
        ]
...
def print_best_individual(generation, best_individual, fitness):
    ...
    num_layers, layer_sizes, beta, lr, dropout = parse_individual(
        best_individual
    )
...
def plot_evolution_progress(logbook, generation_best):
    ...
    ax3.hist(
        final_gen_data, bins=15, alpha=0.7, color="purple", edgecolor="black"
    )
    ax3.axvline(
    ...
    ax4.plot(
        generations[1:], improvement_rate, "orange", linewidth=2, marker="d"
    )
...
def plot_architecture_evolution(generation_best):
    ...
    im = ax2.imshow(
        heatmap_data.T, cmap="viridis", aspect="auto", interpolation="nearest"
    )
    ax2.set_xlabel("Generation")
    ...
    for i, (pos, size, color) in enumerate(
        zip(layer_positions, all_sizes, colors)
    ):
...
def plot_beta_evolution(generation_best):
    ...
    ax1.plot(
        generations, betas, "purple", linewidth=3, marker="o", markersize=8
    )
    ax1.set_xlabel("Generation")
...
• Dropout: {best_dropout:.4f}"""
    ...
    ax_depth.plot(
        gen_data, layers_data, "purple", linewidth=2, marker="s", markersize=6
    )
    ...
    ax_params.plot(
        gen_data, param_counts, "orange", linewidth=2, marker="^", markersize=6
    )
    ...
    ax_beta.plot(
        gen_data, beta_data, "red", linewidth=2, marker="o", markersize=6
    )
    ...
    ax_lr.plot(
        gen_data, lr_data, "green", linewidth=2, marker="d", markersize=6
    )
...
Architecture Range: {min(layers_data)}-{max(layers_data)} layers          Parameter Range: {min(param_counts):,}-{max(param_counts):,}          Beta Range: {min(beta_data):.3f}-{max(beta_data):.3f}"""
    ...
    plt.savefig(
        "comprehensive_evolution_summary.png", dpi=300, bbox_inches="tight"
    )
    plt.show()
...
toolbox.register(
    "mutate", bounded_mutation_with_depth, mu=0, sigma=0.1, indpb=0.15
)
